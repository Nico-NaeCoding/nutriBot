# 🥗 NutriBot

<p align="center">
  <b>AI 기반 맞춤 식단 추천 및 영양 상담 챗봇</b><br/>
  <i>AI-Powered Personalized Diet Recommendation Chatbot</i>
</p>

---

## 📌 Overview

**NutriBot**은
사용자의 신체 정보(성별, 나이, 키, 체중 등)를 기반으로 맞춤형 식단을 추천하고 영양 상담을 제공하는 AI 챗봇 서비스입니다.

단순한 챗봇이 아닌

* 👤 사용자 신체 정보 기반 개인화
* 🤖 OpenAI API 기반 자연어 상담
* 🥗 맞춤형 식단 추천
* 💬 대화형 인터페이스 제공

을 결합하여

👉 **사용자가 자신의 건강 상태와 목표에 맞는 식단 정보를 쉽고 자연스럽게 얻을 수 있는 서비스**를 만드는 데 초점을 맞췄습니다.

---

## 👥 Team & Roles

| [전한나](https://github.com/jdy031017js) | [김해진](https://github.com/sunjinnn) | [박소정](https://github.com/sojeong0302) | [이수진](https://github.com/leesujin1209) |
| :---: | :---: | :---: | :---: |
| <img src="https://github.com/jdy031017js.png" width="80"/> | <img src="https://github.com/sunjinnn.png" width="80"/> | <img src="https://github.com/sojeong0302.png" width="80"/> | <img src="https://github.com/leesujin1209.png" width="80"/> |
| **팀장**<br/>**프론트엔드** | **프롬프트<br/>엔지니어링** | **프론트엔드** | **OpenAI API** |

## ✨ Features

### 👤 User Profile

* 성별, 나이, 키, 몸무게 입력
* 사용자 프로필 정보 관리
* 개인화된 상담을 위한 정보 반영

---

### 🤖 AI Diet Consultation

* OpenAI API 기반 식단 상담
* 건강 및 영양 관련 질의응답
* 자연어 기반 대화형 인터페이스

---

### 🥗 Personalized Recommendation

* 사용자 신체 정보 기반 식단 추천
* 건강 상태를 고려한 맞춤형 조언
* 개인화된 식단 예시 제공

---

### 💻 User Experience

* 실시간 채팅 인터페이스
* 직관적인 사용자 입력 UI
* 반응형 웹 지원

---

## 🛠 Tech Stack

### 🎨 Design

![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge\&logo=figma\&logoColor=white)

### 💻 Frontend

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge\&logo=react)
![styled-components](https://img.shields.io/badge/styled--components-DB7093?style=for-the-badge\&logo=styled-components\&logoColor=white)
![Axios](https://img.shields.io/badge/Axios-5A29E4?style=for-the-badge)

### 🤖 AI

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge\&logo=openai\&logoColor=white)

### 🤝 Collaboration

![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge\&logo=github\&logoColor=white)
